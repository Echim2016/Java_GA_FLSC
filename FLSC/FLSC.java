...
/*
    public double fitness(int num_of_chromosome) {
    	...
    	if(the_same == true) {
    		...
    		double tempMax;
    		Pair<Double,Pair<Integer,Integer>> pairPreference;
        	...
        	for(int iteration=0;iteration<costIteration;iteration++) {
                ...
                while(true) {
            		...
            		for(int i=0;i<manNum;i++) {
            			for(int j = 0; j < parkNum; j++){
                            for(int k = 0; k < facilityNum; k++){
                            	pairPreference.setAt0(file.p[i][j][k]);
                                pairPreference.L=file.p[i][j][k];
                                pairPreference.R.L=j;
                                pairPreference.R.R=k;
