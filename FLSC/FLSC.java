...
//    	int[][] myS = file.getS();
    ...
    public static void crossover() {
    	...
    	for(int i=0;i<poolLength;i++) {
    		...
    		System.arraycopy(frontGene[i], 0, newGene[i], 0, pos);
    		System.arraycopy(backGene[shuffleOrder.get(i)], 0, newGene[i], pos, parkNum-pos);
    	}
    	
    	// save the result to parent_pool
    	for(int i=0;i<poolLength;i++) {
    		for(int j=0;j<parkNum;j++) {
    			parent_pool[i][j] = newGene[i][j]; 
    		}
    	}
    }
